...
setOpenAssign(false)
  ...
  const handleCreateTask = () => {
    axios.post(
      `https://localhost:7115/api/v1/task/create?TaskName=${taskName}&Description=${taskDes}&DeadLine=${taskDeadline}&DateEnd=2023-01-30T15%3A25%3A37.917&ProjectId=${projectState.id}&StaffId=${[...selectedStaff]}`
  ...
  const fetchDataTaskList = async () => {
    await axios.get(`https://localhost:7115/api/v1/task/getAllTask`).then((response) => {
      ...
      setTaskList(response.responseSuccess)
...
console.log("project after", projectState)
    ...
    <>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          <TableBody>
            {taskList.filter(tasks => tasks.projectId == projectState.id)
      ...
      <TablePagination
        rowsPerPageOptions={[]}
        component="div"
        count={projectState?.empTasks?.length}
      ...
      {/* deatil task */}
      <DetailTask show={openDetail} close={() => setOpenDetail(false)} task= {taskID} />
      ...
      >
          ...
          <ul style={{ lineHeight: 3.5, listStyle: "none" }}>
            ...
            <li>
              ...
              {selectedOptions.length ? selectedOptions.map((select, index) => (
                <Chip label={select.fullName}></Chip>
              )): <></>}
              <IconButton onClick={handleClickOpenAssign} aria-label="delete">
                <PersonAddAlt1Icon sx={{ color: "#22a19a" }} />
              </IconButton>
        ...
        <DialogActions>
          ...
          <ColorButton size="small" onClick={() => handleCreateTask()} variant="contained">
            Create task
          </ColorButton>
        </DialogActions>
